function [varst, SWres,Bset,FWHMres] = search_tool_V6_4x(KRV,PM,RES,pm_cl, PZ,Mdl_dtl,Bobj,theta,particle_loc,control,con)
    ...
    for pmcount = 1:length(PM)
        ...
        for  count2 = 1:size(KRV,2)
            ...
            while abs(tmps(1) - tmps(2)) > 1e-3 && SH0 > MxB(pm,length(MxB(pm,:))) && tmps(2) ~= 0
                ...
                [FWHMX(1),MLOC(1)] = FWHM_4x(DNVC(1,:),theta);
                %RKout = zeros(1,100); 
